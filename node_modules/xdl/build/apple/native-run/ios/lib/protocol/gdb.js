...
class GDBProtocolReader extends _protocol().ProtocolReader {
  ...
  onData(data) {
    ...
    try {
      ...
      this.buffer = data ? Buffer.concat([this.buffer, data]) : this.buffer; // do we have enough bytes to proceed
      ...
      if (this.parseHeader(this.buffer) === -1) {
        ...
        this.buffer = this.buffer.slice(this.headerSize + body.length); // parse the payload and recurse if there is more data to process
  ...
  parseBody(buffer) {
    debug(`Response body: ${buffer.toString()}`); // check for checksum
...
class GDBProtocolWriter {
  write(socket, msg) {
    ...
    debug(`Socket write: ${cmd}, args: ${args}`); // hex encode and concat all args
...
function calculateChecksum(cmdStr) {
  ...
  let result = (checksum % 256).toString(16); // pad if necessary
...
function validateChecksum(checksum, msg) {
  ...
  const checksumVal = checksum.slice(1); // remove '$' from msg and calculate its checksum
