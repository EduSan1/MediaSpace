 ...
 */
const debug = (0, _debug().default)('expo:xdl:ios:lib:client:mobile_image_mounter');
...
class MobileImageMounterClient extends _client().ServiceClient {
  constructor(socket) {
  ...
  async mountImage(imagePath, imageSig) {
    ...
    if (!(0, _lockdown().isLockdownResponse)(resp) || resp.Status !== 'Complete') {
      throw new (_client().ResponseError)(`There was an error mounting ${imagePath} on device`, resp);
    }
  }
  ...
  async uploadImage(imagePath, imageSig) {
    ...
    return this.protocolClient.sendMessage({
      ...
      if (isMIMUploadReceiveBytesResponse(resp)) {
        ...
        reject(new (_client().ResponseError)(`There was an error uploading image ${imagePath} to the device`, resp));
      }
    });
  }
...
//# sourceMappingURL=mobile_image_mounter.js.map